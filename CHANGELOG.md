...
- **Виправлено зникнення списку серверів після перезапуску (Restore Backup / старт програми):**
  - У `App.tsx` відновлено виклики `serversCtrl.loadServers()`, `serversCtrl.loadFavorites()`, `serversCtrl.loadCustomFolders()` та `vault.checkUnlockStatus()` на початковому монтуванні додатка.
  - До хуків `useServers` та `useCredentials` додано внутрішній `useEffect` автозавантаження даних з бекенду при ініціалізації.
  - При відновленні бекапу (`handleImportBackup`) та імпорті CSV тепер додатково автоматично перезавантажуються папки та список обраного.
- **Виправлено SQLite WAL-синхронізацію при резервному копіюванні та відновленні:**
  - При експорті резервної копії додано примусовий `wal_checkpoint(TRUNCATE)` перед копіюванням файлу, що гарантує потрапляння всіх незафіксованих сторінок WAL-транзакцій у бекап.
  - При відновленні бекапу додано очищення застарілих `rdm.db-wal` та `rdm.db-shm` файлів перед підміною основної БД та повторне ввімкнення WAL-режиму і foreign keys на новому підключенні.

## v2.1.0

- **КРИТИЧНО: в інсталяторах пакувався сайт замість програми.** З коміту «presentation site» (5.09) кореневий `index.html` React-додатка було замінено лендінг-сторінкою для GitHub Pages, тож `vite build` збирав лендінг (2 модулі, 7.6 кБ) і всі релізи v2.0.6–v2.0.9 містили сайт замість додатку. Тепер кореневий `index.html` знову є входом Vite/React, а лендінг перенесено в `site/` (GitHub Pages деплоїть лише його — заодно це прибрало публікацію всіх вихідників репозиторію на Pages)
- **Оновлення всередині програми (in-app auto-update):** додаток тепер сам перевіряє, завантажує та встановлює оновлення — без відкриття браузера. З'являється банер із кнопкою «Install now / Встановити», прогрес-бар завантаження, після встановлення програма сама перезапускається. Fallback на сторінку релізів — лише якщо підписаний updater недоступний (наприклад, portable-версія)
- **Підписані updater-артефакти:** увімкнено `createUpdaterArtifacts`, згенеровано ключі підпису minisign (публічний ключ вшито в `tauri.conf.json`, приватний — у GitHub Secrets як `TAURI_SIGNING_PRIVATE_KEY`). Раніше updater не міг працювати в принципі: pubkey був порожній
- **Виправлено конфіг updater:** прибрано невалідні ключі `"dialog"` і `"active"` (з Tauri v1) — вони ламали десеріалізацію конфігу; режим встановлення на Windows — `passive` (тихе оновлення без діалогів)
- **Прогрес завантаження оновлення:** Rust-команда `install_update` шле подію `update://progress` (завантажено/всього байтів), фронтенд малює прогрес-бар
- **Коректний перезапуск після оновлення:** на Windows процес виходить і NSIS-інсталятор сам перезапускає програму, на macOS/Linux — `app_handle.restart()`
- **Нові інсталятори в релізі:** Windows — NSIS + portable ZIP; Ubuntu/Linux — AppImage + **DEB** (новий); macOS — DMG для Intel (x64) та Apple Silicon (aarch64)
- **CI генерує `latest.json`:** кожна платформа збирає фрагмент (підпис + URL артефакта), фінальний job `publish-updater` зливає їх у `latest.json` і прикріплює до релізу — саме його читає вбудований updater
- Прибрано крок встановлення WiX у CI — MSI не збирався і не був у targets
