...
impl<S: Read + Write> Client<S> {
    ...
    /// ```
    pub fn start_ssl(self, check_certificate: bool, hostname: &str) -> RdpResult<Client<S>> {
        Ok(Client::new(self.transport.start_ssl(check_certificate, hostname)?))
    }
    ...
    /// ```
    pub fn start_nla(self, check_certificate: bool, authentication_protocol: &mut dyn AuthenticationProtocol, restricted_admin_mode: bool, hostname: &str) -> RdpResult<Client<S>> {
        let mut link = self.transport.start_ssl(check_certificate, hostname)?;
