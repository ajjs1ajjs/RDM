...
impl<S: Read + Write> Client<S> {
    ...
    /// ).unwrap()
    /// ```
    pub fn connect(mut tpkt: tpkt::Client<S>, security_protocols: u32, check_certificate: bool, authentication_protocol: Option<&mut dyn AuthenticationProtocol>, restricted_admin_mode: bool, blank_creds: bool, hostname: &str) -> RdpResult<Client<S>> {
        ...
        match Self::read_connection_confirm(&mut tpkt)? {
            Protocols::ProtocolHybrid => Ok(Client::new(tpkt.start_nla(check_certificate, authentication_protocol.unwrap(), restricted_admin_mode || blank_creds, hostname)?,Protocols::ProtocolHybrid)),
            Protocols::ProtocolSSL => Ok(Client::new(tpkt.start_ssl(check_certificate, hostname)?, Protocols::ProtocolSSL)),
